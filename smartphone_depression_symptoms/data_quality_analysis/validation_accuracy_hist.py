...
for t in task:
    ...
    for s in seq:
        ...
        sns.histplot(data=_d2plot.accuracy, kde=False, ax=ax[i_s, i_t], color=colors[i_t][i_s], alpha=1)
        # ax[i_s, i_t].set_title(f'Validation {s}')
        ax[i_s, i_t].set_xlabel(f'{add_space_between_words(t)} - {"First" if s==0 else "Last"} validation', fontsize=12)
        # ax[i_s, i_t].set_y_label("# points", fontsize=12)
        ax[i_s, i_t].text(1.5, 24,
                          f'Median={np.median(_d2plot.accuracy):.2f} cm\nMean={np.mean(_d2plot.accuracy):.2f} cm\n'
                          f'80%={np.quantile(_d2plot.accuracy, 0.8):.2f} cm')
        ax[i_s, i_t].set_ylim([0.0, 30.0])
        ax[i_s, i_t].set_xlim([0, 3])
